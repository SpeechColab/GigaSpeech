...
if [ $# -ne 2 ]; then
  echo "Usage: $0 [options] <gigaspeech-dataset-dir> <GigaSpeech-json> "
  echo " e.g.: $0 --subset XL /disk1/audio_data/gigaspeech /disk1/GigaSpeech.json"
  echo ""
...
if [ "$subset" == "XL" ];then 
  ...
  python3 utils/extract_subset.py \
    $subset $GigaSpeech_json || exit ;
  for line in `cat log/subset_path`
  do
    $ossbin -c SAFEBOX/aliyun_ossutil.cfg \
