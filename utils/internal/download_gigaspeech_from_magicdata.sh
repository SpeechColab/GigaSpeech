...
if [ $# -ne 1 ]; then
  ...
  echo ""
  echo "This script downloads the entire GigaSpeech Dataset from the Magic Data"
...
if [ `uname -s` != 'Linux' ] && [ `uname -s` != 'Darwin' ]; then
  echo "$0: The Magic Data host downloader only supports Linux and Mac OS."
  exit 1
fi

# Check release URL
if [ -z "$GIGASPEECH_RELEASE_URL_MAGICDATA" ]; then
  echo "$0: Error, variable GIGASPEECH_RELEASE_URL_MAGICDATA (in env_vars.sh)"
  echo "$0: is not set."
  exit 1
fi
GIGASPEECH_RELEASE_URL=$GIGASPEECH_RELEASE_URL_MAGICDATA
...
if [ $stage -le 1 ]; then
  ...
  grep -v '^#' misc/magicdata/metadata.list | (while read line; do
...
if [ $stage -le 2 ]; then
  ...
  for audio_source in youtube podcast audiobook; do
    grep -v '^#' misc/magicdata/${audio_source}.list | (while read line; do
      download_object_from_release $line || exit 1;
    done) || exit 1;
  done
fi
...
if [ $with_dict == true ]; then
  if [ $stage -le 3 ]; then
    ...
    grep -v '^#' misc/magicdata/dict.list | (while read line; do
      download_object_from_release $line || exit 1;
    done) || exit 1;
  fi
fi
